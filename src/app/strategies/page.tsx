...
interface Row {
  ...
  m: Record<string, number>; curve?: Curve; hodl?: Curve; alive: boolean; regime: boolean; solBench: boolean; flagship: boolean; leverage?: number;
}
...
function parseRow(c: { _id: string; name: string; stage: string; source: string; composite?: number; failedStage?: string; metrics?: string; curves?: string }): Row {
  ...
  return {
    ...
    flagship: m.flagship === 1, leverage: m.leverage,
  };
}
...
export default function StrategiesPage() {
  ...
  // HERO = the FLAGSHIP CORE-4 portfolio; fall back to any regime strat.
  const hero = mine.find((r) => r.flagship) ?? mine.find((r) => r.regime) ?? rows.find((r) => r.regime) ?? null;
  // other variants (single-coin SOL + proven BTC) shown as secondary cards
  const variants = mine.filter((r) => r.regime && !r.flagship && r.id !== hero?.id);
  const btcVariant = variants.find((r) => !r.solBench) ?? null;
  const solVariant = variants.find((r) => r.solBench) ?? null;
  ...
  // hero numbers (real measured) — flagship CORE-4 vs holding BTC
  ...
  const heroHodlRaw = hero?.hodl ? { t: hero.hodl.t, c: hero.hodl.eq } : null;
  const isFlag = !!hero?.flagship;
  ...
  return (
    <div className="space-y-7 stagger pb-12">
      ...
      <Lead dot="ok">
        {hero && isFlag
          ? <>Your flagship strategy spreads Daniel&apos;s chop-gated trend across <span className="text-fg">4 coins (BTC, ETH, SOL, BNB)</span> at <span className="text-fg">1.45× leverage</span>, and {beatsRet ? <span className="text-up">beat holding Bitcoin</span> : <>made <span className="text-up">{hRet != null ? `+${(hRet * 100).toFixed(0)}%` : "—"}</span></>} at <span className="text-fg">about half the drawdown</span> (−{hDD != null ? `${Math.abs(hDD * 100).toFixed(0)}` : "—"}% vs Bitcoin&apos;s −{hHodlDD != null ? `${Math.abs(hHodlDD * 100).toFixed(0)}` : "—"}%).{" "}
      ...
      {/* ============ HERO: the FLAGSHIP CORE-4 portfolio ============ */}
      {hero && (
        <section className="panel blue-glow-pulse p-7">
          <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
            <div>
              <div className="num text-[11px] blue-glow-text tracking-wider mb-1">★ YOUR STRATEGY — THE FLAGSHIP</div>
              <div className="text-[22px] text-fg font-medium">4-coin chop-trend portfolio · 1.45× leverage</div>
              <div className="num text-[11px] text-dim mt-1 flex flex-wrap items-center gap-1.5">
                <span>BTC + ETH + SOL + BNB, equal weight, each following its trend and sitting out the chop</span>
              </div>
            </div>
            <span className="num text-[10px] px-2.5 py-1 rounded blue-glow-text shrink-0" style={{ background: "#5cc8ff18" }}>{isFlag ? "★ flagship · forward-testing" : "forward-testing"}</span>
          </div>

          {/* the big numbers: backtest vs holding BTC */}
          <div className="grid sm:grid-cols-3 gap-5 mb-6">
            <div className="rounded-xl bg-[#3ddb9e0e] border border-[#3ddb9e22] p-4">
              ...
              <div className="num text-[11px] text-dim mt-1.5">vs <span className="text-accent">+{hHodlRet != null ? `${(hHodlRet * 100).toFixed(0)}` : "—"}%</span> holding Bitcoin</div>
            </div>
            <div className="rounded-xl bg-[#5cc8ff10] border border-[#5cc8ff33] p-4">
              <div className="hud mb-1.5 flex items-center">Worst dip — the edge<Info>Max drawdown — biggest drop from a peak. The 4-coin spread + chop filter keep the drawdown to about half of holding Bitcoin, even at 1.45× leverage.</Info></div>
              <div className="num text-[36px] leading-none blue-glow-text">{hDD != null ? `${(hDD * 100).toFixed(0)}%` : "—"}</div>
              <div className="num text-[11px] text-dim mt-1.5">vs <span className="text-down">{hHodlDD != null ? `${(hHodlDD * 100).toFixed(0)}%` : "—"}</span> holding BTC{ddImprove != null ? ` · ~${(ddImprove * 100).toFixed(0)}% less risk` : ""}</div>
            </div>
            <div className="rounded-xl bg-[#ffffff05] border border-edge p-4">
              <div className="hud mb-1.5 flex items-center">Quality score<Info>Sharpe ratio — return per unit of risk. Above ~1 is good; this portfolio scores ~1.1.</Info></div>
              <div className="num text-[36px] leading-none text-fg">{fmt(hero.oos)}</div>
              <div className="num text-[11px] text-dim mt-1.5">Sharpe · Calmar {hero.m.fullCalmar != null ? hero.m.fullCalmar.toFixed(2) : "—"}</div>
            </div>
          </div>

          <div className="num text-[11px] text-accent mb-3">
            Historical backtest over ~{windowYrs}y at <span className="blue-glow-text">1.45× leverage</span> (funding + liquidation modeled; at 1.45× a wipe-out needs a ~−69% intraday crash — far off, so no liquidation risk). <span className="text-dim">Honest: forward drawdown runs DEEPER than backtest — the real −{hDD != null ? Math.abs(hDD*100).toFixed(0) : "26"}% could run to ~−30%+ live. It&apos;s not a hard cap. Backtest ≠ live.</span> The live test is on the <Link href="/" className="blue-glow-text hover:underline">Live tab</Link>.
          </div>
          ...
          {headline ? (
            <ChartWithBenchmarks
              ...
              extra={heroHodlRaw ? [{ label: "BTC HODL", color: "#f5b932", raw: heroHodlRaw, primary: true }] : []}
              stratLabel="your strategy (4-coin)"
              series={[{ name: "your strategy (4-coin)", color: "#5cc8ff", curve: headline }]}
            />
          ) : <div className="well flex items-center justify-center" style={{ height: 300 }}><span className="hud">no curve</span></div>}
        </section>
      )}

      {/* ============ other variants (single-coin BTC proven + SOL) ============ */}
      {(btcVariant || solVariant) && (
        <section className="panel p-5">
          <div className="hud mb-3">Other variants of your strategy</div>
          <div className="grid sm:grid-cols-2 gap-3">
            {btcVariant && (
              <div className="rounded-lg border border-edge/60 p-3.5">
                <div className="num text-[10px] blue-glow-text mb-1">single-coin BITCOIN — the PROVEN one</div>
                <div className="num text-[11px] text-mid">Backtest <span className="text-up">+{totalOf(btcVariant) != null ? (totalOf(btcVariant)! * 100).toFixed(0) : "—"}%</span> vs BTC <span className="text-accent">+{btcVariant.m.hodlTotal != null ? (btcVariant.m.hodlTotal * 100).toFixed(0) : "—"}%</span> · dip <span className="blue-glow-text">{btcVariant.m.fullMaxDD != null ? (btcVariant.m.fullMaxDD * 100).toFixed(0) : "—"}%</span> · Sharpe {fmt(btcVariant.oos)}</div>
                <div className="flex items-center gap-2 mt-2">
                  <span className="num text-[9px] text-up">✓ passed the strict test</span>
                  <button onClick={() => setSelected(btcVariant.id)} className="num text-[9px] px-2 py-0.5 rounded border border-edge text-dim hover:text-fg">chart it ↑</button>
                </div>
              </div>
            )}
            {solVariant && (
              <div className="rounded-lg border border-edge/60 p-3.5">
                <div className="num text-[10px] text-accent mb-1">single-coin SOLANA — crash-dodger</div>
                <div className="num text-[11px] text-mid">Backtest <span className="text-up">+{totalOf(solVariant) != null ? (totalOf(solVariant)! * 100).toFixed(0) : "—"}%</span> vs SOL <span className="text-promo">+{solVariant.m.hodlTotal != null ? (solVariant.m.hodlTotal * 100).toFixed(0) : "—"}%</span> · dip <span className="blue-glow-text">{solVariant.m.fullMaxDD != null ? (solVariant.m.fullMaxDD * 100).toFixed(0) : "—"}%</span> · Sharpe {fmt(solVariant.oos)}</div>
                <div className="flex items-center gap-2 mt-2">
                  <span className="num text-[9px] text-dim">did not pass the strict test</span>
                  <button onClick={() => setSelected(solVariant.id)} className="num text-[9px] px-2 py-0.5 rounded border border-edge text-dim hover:text-fg">chart it ↑</button>
                </div>
              </div>
            )}
          </div>
