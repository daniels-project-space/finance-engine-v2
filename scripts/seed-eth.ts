...
const FULL = process.env.ETH_FULL ?? "/root/nupl-spike/eth_full.json";
const fmt = (t: number) => new Date(t).toISOString().slice(0, 10);

// The validated blend doc (Daniel's config).
const DOC: BlendSleeveDoc = {
  name: "blend_eth_70_30_onchain_trend_seed",
  kind: "blend",
  hypothesis:
    "The same blend strategy as BTC, tuned for ETH. On-chain cycle overlay (accumulate on capitulation NUPL <= 0, distribute into euphoria) + a 30% trend leg, half-size accumulation cap below the 200d MA, and a T-bill yield on idle cash. THE ONE ETH ADJUSTMENT: ETH's on-chain valuation cycle peaks LOWER than BTC's (NUPL tops ~0.57 vs ~0.75), so the euphoria-sell threshold is 0.55 (not 0.60) — otherwise it would never sell and ride ETH's brutal drawdowns. Faster 50d trend leg. Since-2020 backtest ~30x at -57% maxDD / Calmar ~1.2 / Sharpe ~1.2 (vs ETH HODL 12x / -79% / Calmar 0.59) — a big improvement, though ETH is more volatile than BTC so the drawdown stays higher. The sell-0.55 / sma-50 tuning tops the sweep on BOTH the 2020 and 2017 cycles (not overfit). No leverage. NUPL uses the free Coin Metrics ETH proxy. Forward-paper (backtest, not real money).",
  symbol: "ETH/USDT", tf: "1d",
  wOnchain: 0.70, smaWin: 50,
  // ETH ADJUSTMENT: sell at NUPL >= 0.55 (ETH's cycle peaks lower than BTC's 0.60).
  nuplBuy: 0.0, nuplSell: 0.55, maWin: 200, dcaCapDays: 90, sellStep: 0.06,
  // same efficiency upgrades as BTC: half-size accumulation cap below the 200d + idle yield.
  belowMaCap: 0.5, cashYieldApy: 0.035,
  params: {
    wOnchain: { min: 0.5, max: 0.9, default: 0.70 },
    smaWin: { min: 50, max: 250, default: 50, int: true },
  },
...
async function main() {
  ...
  const card = {
    key: "blend_eth",
    name: "On-chain + trend blend — ETH (70/30)",
    tag: "your strategy",
    desc:
      "The same on-chain + trend blend as the BTC strategy, tuned for ETH. It accumulates ETH when on-chain valuation is in capitulation (NUPL low) and distributes into euphoria, with a 30% trend leg, a half-size accumulation cap below the 200-day average, and a T-bill yield on idle cash. The one ETH-specific adjustment: ETH's valuation cycle peaks LOWER than BTC's (NUPL tops ~0.57 vs ~0.75), so it sells into euphoria at 0.55 instead of 0.60 — otherwise it would never take profit and would ride ETH's brutal drawdowns. Net since-2020: about 30x return at -57% max drawdown (vs holding ETH: 12x at -79%), Calmar ~1.2 vs 0.59 — a big improvement, though ETH is far more volatile than BTC so the drawdown stays higher than the BTC sleeve's -20%. The tuning is robust (tops the sweep on both the 2020 and 2017 cycles). No leverage. Uses the free Coin Metrics ETH NUPL proxy. Backtest; live forward drawdowns can run deeper.",
    start: "2020-01",
    leverage: 1,
    hodlLabel: "ETH HODL",
  ...
  // keep the BTC blend (blend7030) and any other cards; add/replace the ETH card AFTER
  // them (BTC stays the first/hero card with the blue glow).
  const others = (cfg.strategies ?? []).filter((s) => s.key !== "blend_eth");
  const next = { generatedAt: Date.now(), strategies: [...others, card] };
  await cx.mutation(api.pipeline.setConfig, { key: "my_strategies", json: JSON.stringify(next) });
  console.log(`  -> wrote My Strategies card "blend_eth" alongside ${others.length} existing card(s)`);
